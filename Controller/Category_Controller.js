...
let all = async (req,res,next)=>{
   let result = await DB.find();
    helper.FMSG(res,"Get All Category Name",result)
}
...
let add = async (req,res,next)=>{
    ...
    if(name){
        ...
        helper.FMSG(res,"Added Category Name",result)
    }
}
...
let get = async (req,res,next)=>{
    let id = await DB.findById(req.params.id);
    if(id){
        let result=await DB.findById(id._id,req.body);
...
module.exports = {all,add,drop,patch,get}